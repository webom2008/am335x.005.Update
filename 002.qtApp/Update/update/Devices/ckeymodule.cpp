...
CKeyModule::CKeyModule(CSerialControl *pSerialControl)
    :CModuleBase(pSerialControl, &m_SerialKeyAnalysis)
...
void CKeyModule::process_ms(const unsigned long long &sysTick)
{
    this->processRxDataList(sysTick);
    this->processTxDataList(sysTick);
}

void CKeyModule::process_s(const unsigned long long &sysTick)
{
    this->processCommStatus(sysTick);
}

void CKeyModule::processTxDataList(const unsigned long long &sysTick)
{
    ...
    if(NULL != this->m_pSerialControl)
    {
        ...
        {
            if(sysTick-it->second.second >= KEY_RESPONSE_TIMEOUT)
            {
...
void CKeyModule::processRxDataList(const unsigned long long &sysTick)
{
...
void CKeyModule::processCommStatus(const unsigned long long &sysTick)
{
...
void CKeyModule::write_txData(const CAnalysisData &txData)
{
    MutexLocker mLocker(txMutex);(void)mLocker;
...
void CKeyModule::delete_txData(const CAnalysisData &rxData)
{
    MutexLocker mLocker(txMutex);(void)mLocker;
    std::map<unsigned char, std::pair<CAnalysisData, unsigned long long> >::iterator it;
...
void CKeyModule::write_rxData(const CAnalysisData &rxData)
{
    MutexLocker mLocker(rxMutex);(void)mLocker;
...
void CKeyModule::doUpdate(const CAnalysisData &data)
{
...
int CKeyModule::initUpdate(const UINT8 &target, const QString &str)
{
    int ret = -1;
    if (NULL != this->m_pUpdateModule)
    {
